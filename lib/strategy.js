...
function Strategy(options, verify) {
  ...
  options.tokenURL = options.tokenURL || 'https://www.googleapis.com/oauth2/v3/token';
  options.userInfoURL = options.userInfoURL || 'https://www.googleapis.com/plus/v1/people/me/openIdConnect';
  options.revocationURL = options.revocationURL || 'https://accounts.google.com/o/oauth2/revoke';
  options.tokenInfoURL = options.tokenInfoURL || 'https://www.googleapis.com/oauth2/v1/tokeninfo';
  options.oidcIssuer = options.oidcIssuer || 'accounts.google.com';
...
Strategy.prototype.revoke = function(options, done) {
  ...
  this.configure(null, function(err, config) {
    ...
    console.log('revoke param=');
    console.log(param);
    oauth2._request("GET", revocationURL + param, { 'Authorization': "Bearer " + options.accessToken, 'Accept': "application/json" }, null, null, function(err, body, res) {
      if ( log_verbose )
      {
        console.log('REVOKE');
        console.log(body);
      ...
      if (err) { return done(new InternalOAuthError('failed to revoke access token', err)); }
      done(null);
    });
  });
}
